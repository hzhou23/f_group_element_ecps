...
/*    commercial use requires licensing by p.spellucci employer:TUD          */
...
/*    purposes                                                               */
/*    commercial uses require permission and licensing from TUD.             */
/*    for this contact p.spellucci at                                        */
/*    spellucci _at_  mathematik.tu-darmstadt.de                             */
/* d o n l p  2                                                              */
/*    version  5/27/2009 (*)                                                 */
...
/* ************************************************************************* */
/* (*) modifying and extending  a c-version                                  */
...
/*     with P. Spellucci resp. TUD                                           */
/*     copyright exploitation now owned by Technical University at Darmstadt */
/*     ANY NONACADEMIC USE WITHOUT LICENSE FROM TUD CONSTITUTES A VIOLATION  */
/*     OF TUD RIGHTS AND WILL BE PURSUED                                     */
/* ************************************************************************  */
/*  new function format                                                      */
...
/*  fix of problem with aalist (double restart produced segfault)            */
/*  fix of problem with fusave: no deallocation, wrong reallocation in case  */
/*  of multiple calls                                                        */
/*  bug fix in o8qpdu after L2000: redefining w[i]=w1[i] replaced by         */
/*  w[2*i-1]=w1[2*i-1] and w[2*i]=w1[2*i] etc                                */
/*  17/11/2009 : changed: taubnd no longer needed                            */
/*  near bounds of the x[i] we use one sided differences instead             */
...
/*  default values for low and up , can and should be changed by user in user_init */
...
/*        remember: this is called after user_init                              */
/* **************************************************************************** */
void o8st(void) {
    ...
    /* larger than zero                                         */
    /* here computed approxiamtely only                         */
    /*                        ***** warning *****                        */
    ...
    /* to prevent this. this makes the code unnecessarily slow            */
    /* hence we prefer to use the macros DBL_MIN and DBL_EPSILON          */
...
======================== */
    ...
    /* the macros DB__MIN and DBL_EPSILON are here defined in o8para.h   */
    /* on other systems use the values from float.h                      */
    ...
    rho1   = tm10;
    del01  = del0/tp1;
    delmin = min(delmin,max(tm6*del0,smallw));
...
/*  but remember there are formally 2*nres constraints */
    ...
    icgf   = 0;
    if ( ! silent ) {
        fprintf(prou,"donlp2-intv 18/11/09, copyright P. Spellucci\n");
        ...
        fprintf(meu, "donlp2-intv 18/11/09, copyright P. Spellucci\n");
...
void o8fin(void) {
    ...
    "more than iterma iteration steps",
...
void o8opti(void) 
{
    ...
    /*  remember that  es_routines consider bloc/non bloc call           */
 ...
 /* inactive  
  ...
  /*  active at the new point but not at the old point                        */
    ...
    if ( clow > clwold ) 
    {
        ...
        tauqp = max(one,min(term,taumax));
    }
    if ( tauqp > pow(taufac,3)*tauqp0 ) {
         tauqp = min(pow(taufac,3)*tauqp0,taumax);
    }
...
void o8elim(void) {
    ...
    static INTEGER  i,j,k,l,i1,icur,ipiv;
...
void o8qpdu(void) 
{
    ...
    static DOUBLE   infe1,s1,s2,tiny,
                    ...
                    diff0;
    static INTEGER  i,j,k,ip,l,incr,nosucc;
    static LOGICAL  wlow;
...
/*    old : error    
      ...
      term1 = max(term1,w[i]);
*/
      if ( w[2*i-1] != w1[2*i-1] || w[2*i] != w1[2*i] ) lastch=itstep;
      w[2*i-1] = w1[2*i-1];
      w[2*i] = w1[2*i];
      term1 = max(term1,max(w[2*i-1],w[2*i]));
    }
    psist = su2;
    psi   = su1;
...
/******   new numerical differentiation codes ******************************/
...
void esgradf(DOUBLE x[],DOUBLE gradf[]) 
{
    ...
    static DOUBLE   d1,d2,d3,sd1,sd2,sd3,fhelp,fhelp1,fhelp2,
                    fhelp3,fhelp4,fhelp5,fhelp6,xincr,xhelp,floc,span,sum,floc0;
    ...
    {
        if ( valid ) 
        { /* fugrad comes from user_eval and eval_extern also if analyt=FALSE */
            for (j = 1 ; j <= n ; j++) 
            {
    ...
    else 
    {
        ...
        else 
        {
            if ( difftype == 1 ) 
            {
                ...
                if ( ffuerr )
                {
                  fprintf(stderr,"cannot evaluate f at current x! \n");
                  exit(1);
                }
                ...
                for (j = 1 ; j <= n ; j++) 
                {
                  ...
                  {
                    xhelp = xtr[j]; span = up[j] - low[j] ;
                    xincr = min(min(tm2,deldif*(fabs(xhelp)+1.0)),span/2.0);
                    if ( xhelp >= low[j] && xhelp <=up[j]-xincr )
                    {  /* xhelp in the left half of its interval */ 
                    
                        xtr[j] = xhelp+xincr;
                        ef(xtr,&fhelp);
                        if ( ffuerr ) 
                        {
                          fprintf(stderr,"cannot evaluate f for num diff!\n");
                          exit(1);
                        }
                        gradf[j] = (fhelp-floc)/xincr;
                        xtr[j]   = xhelp;
                     }
                     else /* xhelp in the right half */
                     {
                        xtr[j] = xhelp - xincr ;
                        ef(xtr,&fhelp);
                        if ( ffuerr )  
            ...
            else if ( difftype == 2 ) 
            {
                ...
                /* floc0 used for the unsymmetric form, evaluated once here */
                for (j = 1 ; j <= n ; j++) 
                {
                  if ( low[j] == up[j] )
                  {
                  ...
                  else
                  {
                    span=up[j] - low[j] ;
                    xhelp  = xtr[j];
                    xincr  = min(min(tm2,deldif*(fabs(xhelp)+1.0)),span/4.0);
                    ...
                    {  /* xhelp at the left border */
                       ...
                       gradf[j] = ( -3.0*floc0+4.0*fhelp1-fhelp2)/(2.0*xincr);
                       xtr[j]=xhelp;
                    }
                    else
                    { /* xtr[j] at the right border */
                      xtr[j] = xhelp-xincr;
                      ef(xtr,&fhelp1);
                      if ( ffuerr )  
                      {
            ...
            else   /* difftype ==3 */ 
            {
                ...
                for (j = 1 ; j <= n ; j++) 
                {
                  ...
                  else
                  {
                    ...
                    {  /* xtr[j] not near any bound ; use symmetric code */
                      xtr[j] = xhelp-xincr;
                      ef(xtr,&fhelp1);
                      if ( ffuerr )  
                      {
                    ...
                    { /* xtr[j] near the left border */
                      ...
                      gradf[j] = sum/(5040.0*xincr);
                      xtr[j] = xhelp;
                    }
                    else
                    { /* xtr[j] near the right border */
                      sum = 0.0;
                      for ( k=0; k<=6; k++ )
                      { 
                        xtr[j] = xhelp - k*xincr;
...
static DOUBLE   d1,d2,d3,sd1,sd2,sd3,
                xincr,xhelp,span,sum;
static DOUBLE dcoef[7] = { -12348.0,30240.0,-37800.0,33600.0,-18900.0,
                    6048.0,-840.0};
/* coefficients for sixth order one sided finite difference quotient */
/* common denominator 5040.0 * stepsize */
...
/*  given by liste                                                          */
    ...
    if ( bloc ) 
    {
        ...
        {  /* fugrad comes from user_eval and eval_extern, also if analyt=FALSE*/
           for ( i = 1 ; i <= liste[0] ; i++ )
           {
             for (j = 1 ; j <= n ; j++) 
             {
                /* fugrad has been evaluated in user_eval already */
    ...
    else 
    {
       ...
       else 
       {
          ...
          {
            deldif = min(tm1*sqrt(epsfcn),tm2);
            econ(2,liste,xtr,escongrad_fhelp[1],escongrad_errloc);
            for ( i = 1 ; i <= liste[0] ; i++ )
            {
            ...
            for (j = 1 ; j <= n ; j++) 
            {
              if ( low[j] == up[j] ) 
              {
                 for ( i=1; i <= liste[0] ; i++ )
                 {
                   grad_constr[j][shift+liste[i]] = 0.0 ;
                 }
              }
              else
              {
                ...
                {
                  xtr[j] = xhelp+xincr ;
                  econ(2,liste,xtr,escongrad_fhelp[2],escongrad_errloc);  
                  for ( i = 1 ; i <= liste[0] ; i++ )
                  {
                    if ( escongrad_errloc[liste[i]] )
                    {
                  ...
                  } /* end check error indicator */
                  for ( i = 1 ; i <= liste[0] ; i++ )
                  {
                    grad_constr[j][shift+liste[i]] = 
                    xsc[j]*(escongrad_fhelp[2][liste[i]]
                    -escongrad_fhelp[1][liste[i]])/(xincr);
                  }
                  xtr[j] = xhelp ;
                }
                else
          ...
          {
            deldif = min(tm1*pow(epsfcn,one/three),tm2);
            econ(2,liste,xtr,escongrad_fhelp[0],escongrad_errloc);
            for ( i = 1 ; i <= liste[0] ; i++ )
            {
              if ( escongrad_errloc[liste[i]] )
              {
            ...
            for (j = 1 ; j <= n ; j++) 
            {
              ...
              else
              {
                ...
                if ( xhelp - xincr >= low[j] && xhelp + xincr <= up[j] )
                {
                  ...
                  {      
                    grad_constr[j][shift+liste[i]] = 
                     xsc[j]*(escongrad_fhelp[1][liste[i]]-
                       escongrad_fhelp[2][liste[i]])/(xincr+xincr);
                  }
                  xtr[j]    = xhelp;
                ...
                { /* one sided difference quotient order 2, left*/
                  ...
                  {
                    grad_constr[j][shift+liste[i]] =
                     xsc[j]*(-3.0*escongrad_fhelp[0][liste[i]]+
                       4.0*escongrad_fhelp[1][liste[i]]
                       -escongrad_fhelp[2][liste[i]])/(xincr+xincr);
                  }
                  xtr[j]    = xhelp;
            ...
            for (j = 1 ; j <= n ; j++) 
            {
              ...
              else
              {
                ...
                { /* use central differences */
                  ...
                  d1     = xincr;
                  xtr[j] = xhelp-xincr;
                  econ(2,liste,xtr,escongrad_fhelp[3],escongrad_errloc);
                  for ( i = 1 ; i <= liste[0] ; i++ )
                  {
                    if ( escongrad_errloc[liste[i]] )
                    {
                  ...
                  xtr[j] = xhelp+xincr;
                  econ(2,liste,xtr,escongrad_fhelp[4],escongrad_errloc);
                  for ( i = 1 ; i <= liste[0] ; i++ )
                  {
                    if ( escongrad_errloc[liste[i]] )
                    {
                  ...
                  econ(2,liste,xtr,escongrad_fhelp[5],escongrad_errloc);
                  for ( i = 1 ; i <= liste[0] ; i++ )
                  {
                    if ( escongrad_errloc[liste[i]] )
                    {
                  ...
                  xtr[j] = xhelp+xincr;
                  econ(2,liste,xtr,escongrad_fhelp[6],escongrad_errloc);
                  for ( i = 1 ; i <= liste[0] ; i++ )
                  {
                    if ( escongrad_errloc[liste[i]] )
                    {
                  ...
                  for ( i = 1 ; i <= liste[0] ; i++ )
                  {
                     sd1 = (escongrad_fhelp[2][liste[i]]
                            -escongrad_fhelp[1][liste[i]])/d1;
                     sd2 = (escongrad_fhelp[4][liste[i]]
                            -escongrad_fhelp[3][liste[i]])/d2;
                     sd3 = (escongrad_fhelp[6][liste[i]]
                           -escongrad_fhelp[5][liste[i]])/d3;
                     sd3 = sd2-sd3;
                     sd2 = sd1-sd2;
                     sd3 = sd2-sd3; 
                     grad_constr[j][shift+liste[i]] =
                ...
                { /* xtr[j] near the left end of its interval */
                   for ( k=0; k<= 6; k++ )
                   {
                     ...
                     if ( k >= 1 )
                     {
                       econ(2,liste,xtr,escongrad_fhelp[k],escongrad_errloc);
                       for ( i = 1 ; i <= liste[0] ; i++ )
                       {
                         if ( escongrad_errloc[liste[i]] )
                         {
                   ...
                   for ( i = 1 ; i <= liste[0] ; i++)
                   {
                     ...
                     for ( k=0; k<=6; k++)
                     {
                       sum += dcoef[k]*escongrad_fhelp[k][liste[i]];
                     }
                     grad_constr[j][shift+liste[i]] = xsc[j]*sum/(5040.0*xincr);
                   }
                   xtr[j]    = xhelp;
                }
                else 
...
/*    if ( bloc ) { */ 
    ...
    fud        = d2_malloc(7, nlin+nonlin+1, 1);
/*    }*/
    /* o8fuco.h */
...
/* printf("d_borders_count: %d\n", d_borders_count); */
    ...
    o8qpdu_cii    = d1_malloc(ndualm+1, 1); 
    ...
    escongrad_fhelp = d2_malloc(7,nlin+nonlin+1, 1);
