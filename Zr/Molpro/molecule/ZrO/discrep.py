...
ecps = ['UC', 'crenbl', 'lanl2', 'mdfstu', 'mwbstu','sbkjc','ECP2','ECP3','ECP5']#, 'w3', 'w6', 'w9']#,'i0','i7', 'i6']#'sub0','smal-se3','se3','se4']
styles = {
...
'sbkjc'     :{'label': 'SBKJC',    'color':'#00CDCD','linestyle':'-','dashes': (3,1,1,2) },
...
'ECP2'      :{'label': 'ECP2',    'color':'#e41a1c','linestyle':'--','dashes': (4,2,1,2) },
'ECP3'      :{'label': 'ECP3',    'color':'#cc0099','linestyle':'--','dashes': (4,2,1,2) },
'ECP5'      :{'label': 'ECP5',    'color':'#39e600','linestyle':'--','dashes': (4,2,1,2) },
...
def plot():
    ...
    data = get_data()
    #print data.head()
    data.to_csv("ZrO_TZ.csv", sep=',', index=False)
    ...
    ax.set_xlim((1.20,2.50))
#    ax.set_ylim((-0.25,0.25))
    ax.set(title='ZrO tz Discrepancies')
    #plt.legend(bbox_to_anchor=(0.53, 0.15, 0.5, 0.5), fontsize="x-small")
    plt.legend(loc='best',ncol=2,prop={'size': 15})
    plt.axvline(1.7053960522111073,ls='--',color='gray',linewidth=1.0)
    plt.savefig('ZrO_TZ.pdf')
    plt.show()
