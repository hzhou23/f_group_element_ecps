...
ecps = ['UC', 'crenbl', 'lanl2', 'mdfstu','sbkjc','mwbstu','ECP1','ECP4']#, 'w3', 'w6', 'w9']#,'i0','i7', 'i6']#'sub0','smal-se3','se3','se4']
styles={
...
'ECP4'	        :{'label':'ECP4',	'color':'#009900','linestyle':'--','dashes':(8,1,1,1,1,1)},
'ECP1'          :{'label':'ECP1',   'color':'#8A3324','linestyle':'--','dashes':(8,1,1,1,1,1)},
}
...
def plot():
    ...
    data = get_data()
    #print data.head()
    data.to_csv("ReH_TZ.csv", sep=',', index=False)
    ...
    ax.set_xlim((1.40,2.00))
#    ax.set_ylim((-0.25,0.25))
    ax.set(title='ReH tz Discrepancies')
    #plt.legend(bbox_to_anchor=(0.53, 0.15, 0.5, 0.5), fontsize="x-small")
    plt.legend(loc='best',ncol=2,prop={'size': 15})
    plt.axvline(1.5968049569960796,ls='--',color='gray',linewidth=1.0)
    plt.savefig('ReH_TZ.pdf')
    plt.show()
