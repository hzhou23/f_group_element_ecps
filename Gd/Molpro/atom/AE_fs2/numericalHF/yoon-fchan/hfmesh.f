	...
	dimension ek(40),phe(2000,40),orb(2000,40),njrc(4),vi(2000,11)
...
c  this was added for working with lmitas's code...
	...
	if (ichar.eq.acg) then
	  ...
	  njrc(4)=1
	  do 52 i=1,nr
	  call pseudg(r(i),vs,vp,vd,vf,vloc,vloc1)
	  vi(i,1)=vs+vloc
	  vi(i,3)=vp+vloc
	  vi(i,5)=vd+vloc
	  vi(i,7)=vf+vloc
	...
	write (19,*) r(i),vf
	write (20,*) r(i),vloc1
	write (21,*) r(i),vs+vloc
	write (22,*) r(i),vp+vloc
	write (23,*) r(i),vd+vloc
	write (24,*) r(i),vf+vloc
			endif
 52	  continue
		rcutm=3.
		nmesh=3001
		...
		call pseudg(rmesh,vs,vp,vd,vf,vloc,vloc1)
	write (25,*) rmesh,2.*(vs+vloc),2.*(vp+vloc),2.*(vd+vloc)
        write (25,*) 2.*(vf+vloc),2.*vloc
		enddo
	close(16)
        close(17)
        close(18)
...
c  effects of the pseudo core charge.
	...
	dimension ek(40),phe(2000,40),dv(2,2000),njrc(4),vi(2000,11)
...
c  for inclusion of an orbital dependent potential.
	...
	dimension q0(2000),xm1(2000),xm2(2000),njrc(4),vi(2000,11)
